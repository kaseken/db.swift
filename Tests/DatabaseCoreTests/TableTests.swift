...
struct TableTests {
    @Test func `insert and select one row`() {
        let table = Table()
        let row = Row(id: 1, username: "cstack", email: "foo@bar.com")
    ...
    @Test func `insert and select two rows`() {
        let table = Table()
        let row1 = Row(id: 1, username: "cstack", email: "foo@bar.com")
    ...
    @Test func `table full error`() {
        ...
        for i in 0 ..< Table.maxRows {
            let row = Row(id: UInt32(i), username: "u", email: "e@e.com")
            #expect(table.insert(row: row) == .success)
        }
        let extra = Row(id: 9999, username: "over", email: "over@limit.com")
